...
export const FavoritesButton: React.FC<ButtonProps> = ({
  ...
  accentColor = "red",
	...
	const handleFavorite = () => {
		...
		favorites: isFavorite ? arrayRemove(user?.email || "") : arrayUnion(user?.email || ""),
		}, { merge: true })
		.then(() => {
  ...
  const iconFontSize = String(Number(fontSize.replace(/[^0-9]/g, "")) * 1.5) + fontSize.replace(/[0-9]/g, "");
	...
	return (
      ...
      borderRadius="5px"
		...
		>
			...
			{isFavorite ? <AiFillHeart fontSize={iconFontSize} color={accentColor}/> : <AiOutlineHeart fontSize={iconFontSize} />}
			<Box ml="5px" display="inline-block"> </Box>
